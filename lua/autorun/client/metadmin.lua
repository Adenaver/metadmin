...
function metadmin.menu()
	...
	playerslist.OnClickLine = function(panel,line)
		...
		playerslist:AddColumn("Группа"):SetFixedWidth(220)
...
function metadmin.profile(tab)
	...
	if Access("ma.synch") or Access("ma.refsynch") then
		...
		if Access("ma.synch") then
			...
			row:SetIcon(tab.synch and "icon16/delete.png" or "icon16/accept.png")
		end
		if Access("ma.refsynch") and tab.synch then
			local row = menu:AddOption("Обновить данные", function()
				net.Start("metadmin.synch")
					net.WriteBool(true)
					net.WriteString(tab.SID)
				net.SendToServer()
			...
			row:SetIcon("icon16/arrow_rotate_anticlockwise.png")
		end
		local row = menu:AddOption("Отмена")
		row:SetIcon("icon16/cancel.png")
		menu.Remove = function(m)
